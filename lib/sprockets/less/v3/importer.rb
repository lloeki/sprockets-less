...
module Sprockets
  module Less
    module V3
      # class used for importing files from LES files
      class Importer < Sprockets::Less::V2::Importer


        def resolve_path_with_load_paths(context, path, root_path, file)
          context.resolve(file.to_s, load_paths: context.environment.paths, base_path: root_path, accept: syntax_mime_type)
        rescue
          nil
        end
        ...
        def process(processors, context, path)
          ...
          data = Sprockets::Less::Utils.read_template_file(path.to_s)
        ...
        # because we need to let the Less parser handle that.
